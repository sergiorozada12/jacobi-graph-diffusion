...
class DiffusionScoreModule(DiffusionBaseModule):
    def __init__(self, cfg, sampling_metrics, ref_metrics, node_dist):
        train_loss = MaskedMSELoss(weight=getattr(cfg.train, "lambda_train", 1.0))
        super().__init__(
            cfg,
            sampling_metrics,
            ref_metrics,
    ...
    def _training_step_impl(self, batch_idx, X, adj, observed_mask):
        ...
        target_score = self._analytic_jacobi_score(adj.float(), batch["adj_t"], batch["t"]).to(pred_score.dtype)
    ...
    def _validation_step_impl(self, X, adj, observed_mask):
        ...
        with torch.no_grad():
            ...
            target_score = self._analytic_jacobi_score(adj.float(), batch["adj_t"], t).to(pred_score.dtype)
            mask = self._edge_mask(flags)
            scale = self.train_loss.build_scale(target_score)
            rel_mse = MaskedMSELoss.masked_mse(
                pred_score,
                target_score,
                mask,
                scale=scale,
            ...
            raw_mse = MaskedMSELoss.masked_mse(pred_score, target_score, mask).detach()
